...
{
    public class GovUSAPetroleumAndOthersSupplyDisposition : ExcelXlsxScrapeData
    {
        public const string URL = "https://www.eia.gov/outlooks/aeo/excel/aeotab_11.xlsx";
        ...
        protected override void ScrapeXlsx(ScraperDbContext dbContext, DateTime lastModified, XLWorkbook xlsx)
        {
            ...
            for (int rowIdx = firstRowWithData; rowIdx <= lastRow; rowIdx++)
            {
                ...
                for (int cellIdx = cellShift; cellIdx <= lastCellWithData; cellIdx++)
                {
                    ...
                    string prefixName = "";
                    if (rowIdx >= 16 && rowIdx <= 50)
                        prefixName = "Crude Oil ";
                    if (rowIdx >= 55 && rowIdx <= 62)
                        prefixName = "Product Supplied by Fuel ";
                    if (rowIdx >= 64 && rowIdx <= 69)
                        prefixName = "Product Supplied by Sector ";
                    if (rowIdx == 77)
                        prefixName = "Expenditures for Imported Crude Oil and ";
